...
class Insetter private constructor(
            ...
            when (consume) {
                ...
                CONSUME_AUTO -> {
                    WindowInsetsCompat.Builder(insets)
                        ...
                        .consumeType(
                            Type.navigationBars(), insets, persistentTypes, ignoreVisibility
                        ).consumeType(Type.ime(), insets, persistentTypes, ignoreVisibility)
                        .consumeType(
                            Type.systemGestures(), insets, persistentTypes, ignoreVisibility
                        ).consumeType(
                            Type.displayCutout(), insets, persistentTypes, ignoreVisibility
                        ).build()
                }
                    ...
                    override fun onEnd(animation: WindowInsetsAnimationCompat) {
                        if (currentlyDeferredTypes and animation.typeMask != 0) {
                            ...
                            lastWindowInsets?.let {
                                ViewCompat.dispatchApplyWindowInsets(view, it)
                            }
                        }
    ...
    fun applyInsetsToView(
        view: View, insets: WindowInsetsCompat, initialState: ViewState
    ) {
        if (Log.isLoggable(TAG, Log.DEBUG)) {
            Timber.d(TAG, "applyInsetsToView. View: $view. Insets: $insets. State: $initialState")
        }
    ...
    companion object {
        ...
        @Suppress(
            "NOTHING_TO_INLINE", "DeprecatedCallableAddReplaceWith", "unused", "UNUSED_PARAMETER"
        )
        @Deprecated(
...
internal class SideApply {
    @Sides var left: Int = Side.NONE
        private set

    @Sides var top: Int = Side.NONE
        private set

    @Sides var right: Int = Side.NONE
        private set

    @Sides var bottom: Int = Side.NONE
        private set
...
private fun View.applyPadding(
    ...
    val paddingBottom = when (typesToApply.bottom) {
        Side.NONE -> paddingBottom
            ...
            typesToApply.bottom, ignoreVisibility
        ).bottom
    }
...
private fun View.applyMargins(
    ...
    require(lp is MarginLayoutParams) {
        "Margin window insets handling requested but View's" + " LayoutParams do not extend MarginLayoutParams"
    }
...
private fun WindowInsetsCompat.getInsets(
    @Type.InsetsType typeMask: Int, ignoreVisibility: Boolean
): Insets {
